...
int main()
{
	...
	while (tc--)
	{
		...
		set<int>mx, mn;
		int tallest = *max_element(v.begin(), v.end());
		int shortest = *min_element(v.begin(), v.end());
		for (int i = 0;i < n;i++)
		{
			if (v[i] == tallest)mx.insert(i);
			else if (v[i] == shortest)mn.insert(i);
		}
		int mn_dis = n+1;
		for (auto it : mx)
		{
			auto itt = mn.upper_bound(it);
			if (itt != mn.end())mn_dis = min(mn_dis, (*itt-it));
			if (itt != mn.begin())mn_dis = min(mn_dis, (it - *(--itt)));

		}
		mn.empty()?cout<<0:cout << mn_dis;
