...
int main()
{
	...
	while (t--)
	{
		...
		for (int i = 0; i < n; i++)
		{
			if (v[i] == 4)
			{
				auto  a = mp[8].upper_bound(i);
				if (a == mp[8].end())continue;

				auto  b = mp[15].upper_bound(*a);
				mp[8].erase(a);
				if (b == mp[15].end())continue;

				auto  c = mp[16].upper_bound(*b);
				mp[15].erase(b);
				if (c == mp[16].end())continue;

				auto  d = mp[23].upper_bound(*c);
				mp[16].erase(c);
				if (d == mp[23].end())continue;

				auto  e = mp[42].upper_bound(*d);
				mp[23].erase(d);
				if (e == mp[42].end())continue;
				mp[42].erase(e);
				cnt++;
